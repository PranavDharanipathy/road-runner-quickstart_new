...
public class Areas_BLUE_Bot3Driver extends OpMode {
    ...
    private final static int SLIDES_UP_POSITION = 850;
    ...
    private double outtakeSpeed = -0.8;
    ...
    public void modes() {
        ...
        if (allowIntaking & wantedColor != OurColorSensor.DetectedColor.OTHER) {
            ...
            if (gamepad2.right_trigger > 0.1 && CanIntake) {
                ...
                intake.setPower(outtakeSpeed); //outtake
    ...
    public void extendo() {
        ...
        if (allowExtendo) {
            if (gamepad2.dpad_down) {
                toggle_extendo = false;
                extendoRetract();
            } else if (cur_extendo && !prev_extendo && scoreSampleStages == 0 && pickSpecimenAfterScoringStages == 0 && scoreSpecimenStages == 0 && pickSpecimenStages == 0 && transferToClawStages == 0) {
                if (toggle_extendo) {
                    /** OUT **/
                    extendoExtend();
                    outtakeSpeed = -0.475;
    ...
    public void withinLoopProcessing() {
        ...
        if (scoreSpecimenStages == 1 && scoreSpecimenTimer.milliseconds() >= 1) {
            claw_pivot_servo.setPosition(0.8875);
            scoreSpecimenStages++;
        }
        ...
        if (scoreSpecimenStages == 3 && scoreSpecimenTimer.milliseconds() >= 270) {
            slidesToPose(SLIDES_SCORE_SPEC_POSITION);
            scoreSpecimenStages++;
        }
        ...
        if (pickSpecimenStages == 1 && pickSpecimenTimer.milliseconds() >= 1) {
            ...
            claw_pivot_servo.setPosition(0.875); //goes a bit down before releasing to prevent the sample from getting flung out
            claw_servo.setPosition(0.2);
            pickSpecimenStages++;
        }
